...
/**
 * Non-blocking TCP syslog sink with an in-memory FIFO ring buffer.
 *
 * - Host is fixed to ns2.owo.systems
 * - TCP only
 * - Queue is capped at 5MB and overwrites oldest entries on overflow
 */
class TcpSyslogSink(
    ...
    fun start() {
        synchronized(monitor) {
            if (running) return
            running = true
            worker = Thread({ runWorker() }, "OAL-App-Syslog").apply {
                isDaemon = true
                start()
            }
        }
    ...
    fun enqueue(level: DiagnosticLevel, tag: String, message: String) {
        val line = format(level, tag, message).toByteArray(Charsets.UTF_8)
        synchronized(monitor) {
            if (!running) return
            queue.addLast(line)
    ...
    private fun format(level: DiagnosticLevel, tag: String, message: String): String {
        val pri = when (level) {
            DiagnosticLevel.DEBUG -> 7
            DiagnosticLevel.INFO -> 6
            DiagnosticLevel.WARN -> 4
